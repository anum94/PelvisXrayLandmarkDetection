...
class UNet(nn.Module):
    def contracting_block(self, in_channels, out_channels, kernel_size=3,padding = 1):
        ...
        return block
    def reducing_block(self, in_channels, out_channels, kernel_size=3,padding = 1, stride = 2):
        block = nn.Sequential(
        ...
        return block
    def expansive_block(self, in_channels, mid_channel, out_channels, kernel_size=3, stride = 2):
            block = nn.Sequential(
                   ...
                   # nn.Conv2d(kernel_size=kernel_size, in_channels=mid_channel, out_channels=mid_channel, padding = 'same'),
                    #nn.ReLU(),
                    #nn.BatchNorm2d(mid_channel),
                    nn.ConvTranspose2d(in_channels=mid_channel, out_channels=out_channels, kernel_size=3, stride=stride, padding=1, output_padding=1)
                    #nn.Conv2d(kernel_size=kernel_size, in_channels=in_channels, out_channels=out_channels,padding=1, stride = stride),
                    )
            return  block
