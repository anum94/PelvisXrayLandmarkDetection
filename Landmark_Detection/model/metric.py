...
def percentage_correct_keypoints(output, target):
    ...
    distance_threshold = (
        np.linalg.norm(target.shape[2:4]) * 0.1
    )  # 10% of image diagonal as distance_threshold
    target_landmarks_batch = [
        [
            np.unravel_index(np.argmax(i_target[idx], axis=None), i_target[idx].shape)
            for idx in range(i_target.shape[0])
        ]
    ...
    pred_landmarks_batch = [
        [
            np.unravel_index(np.argmax(i_output[idx], axis=None), i_output[idx].shape)
            for idx in range(i_output.shape[0])
        ]
        for i_output in predictions
    ]

    for idx, (pred_landmarks, target_landmarks) in enumerate(
        zip(np.array(pred_landmarks_batch), np.array(target_landmarks_batch))
    ):
        for channel_idx, (pred_landmark, target_landmark) in enumerate(
            zip(pred_landmarks, target_landmarks)
        ):
            # check if either landmark is correctly predicted as not given OR predicted landmarks is within radius
            if (
                predictions[idx, channel_idx, pred_landmark[0], pred_landmark[1]]
                <= threshold
