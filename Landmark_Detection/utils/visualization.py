...
def compare_output_target(data_imgs, outputs, targets):
    ...
    for sample_idx in range(outputs.shape[0]):
        for channel_idx in range(outputs[sample_idx].shape[0]):
            imgs_rgb[sample_idx,0] = torch.max(imgs_rgb[sample_idx,0],targets[sample_idx,channel_idx])
            imgs_rgb[sample_idx,1] = torch.max(imgs_rgb[sample_idx,1],outputs[sample_idx,channel_idx])
